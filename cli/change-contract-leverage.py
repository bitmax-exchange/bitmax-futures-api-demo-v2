...
@click.option("--leverage", type=str, default=1, help="1 to 100")
@click.option('--verbose/--no-verbose', default=False)
def run(config, botname, symbol, leverage, verbose):
    ...
    url = f"{host}/{group}/api/pro/v2/futures/leverage"
    ...
    headers = make_auth_headers(ts, "v2/futures/leverage", apikey, secret)
